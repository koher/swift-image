    ...
    extension Image where Pixel == RGBA<Float> {
        ...
        private init(width: Int, height: Int, setUp: (CGContext) -> ()) {
            ...
            self.init(width: width, height: height, pixels: pixels.map { RGBA<Float>($0.map { $0 / 255 }) })
        }
    ...
    extension Image where Pixel == RGBA<Double> {
        ...
        private init(width: Int, height: Int, setUp: (CGContext) -> ()) {
            ...
            self.init(width: width, height: height, pixels: pixels.map { RGBA<Double>($0.map { $0 / 255 }) })
        }
